...
setTimeout(() => {
  console.log(child.managedState === ManagedState.DESTROYED); // => true
}, 1);
```
...
class MyListComponent extends Component {
  @managedChild
  list = new ManagedList(new MyComponent(), new MyComponent());
}
...
setTimeout(() => {
  console.log(firstChild.managedState === ManagedState.DESTROYED); // => true
}, 1);
```
...
const view = UIRow.with(
  ...
  UILabel.withText(bindf("You clicked %i times", "count"))
);
