...
function signaturesToString(sig: ReadonlyArray<ts.Signature>, checker: ts.TypeChecker) {
...
export class DeclarationFileParser {
    ...
    getPageData() {
        ...
        let addPage = (topId: string, spec: SpecNode) => {
            ...
            let id = spec.id;
            spec.inherit && spec.inherit.forEach(c => { this._inheritDocRefs(id, c) });
    ...
    private _inheritDocRefs(id: string, clause: string) {
        let baseId = clause.replace(/^(extends|implements)\s*/, "").replace(/\<.*/, "");
        let inheritor = this._nodes[id];
        let base = this._nodes[baseId];
        if (!this._docs[id] || !inheritor || !inheritor.nodes || !base || !base.nodes) return;
        ...
        base.inherit && base.inherit.forEach(c => { this._inheritDocRefs(id, c) });
    }
    ...
    private _addDocumentation(spec: SpecNode, doc?: string | ts.JSDocTag) {
        ...
        if (typeof doc === "string") {
            ...
            switch (doc.tagName.text) {
                case "return": case "returns": result.tag = "return"; break;
                case "note": result.tag = "note"; break;
                case "param":
                    result.tag = "param";
            ...
            if (!this._docs[id].some(d =>
                    d.doc === result.doc && d.tag === result.tag)) {
    ...
    private _createTypeOrEnumSpecNode(id: string, node: ts.TypeAliasDeclaration | ts.EnumDeclaration): SpecNode | undefined {
        ...
        id += (id ? "." : "") + name;
        if (this._nodes[id] && this._nodes[id].type === SpecNodeType.TypeOrEnum) return;
    ...
    private _createFunctionSpecNode(id: string, node: ts.FunctionDeclaration) {
        let symbol = node.name && this.checker.getSymbolAtLocation(node.name);
        let typeSymbol = symbol && this.checker.getTypeOfSymbolAtLocation(symbol, symbol.valueDeclaration);
        ...
        let name = symbol.getName();
        id += (id ? "." : "") + name;
        if (this._nodes[id] && this._nodes[id].type === SpecNodeType.FunctionDeclaration) return;
    ...
    private _createNamespaceSpecNode(id: string, node: ts.NamespaceExportDeclaration): SpecNode | undefined {
        let symbol = node.name && this.checker.getSymbolAtLocation(node.name);
        if (!symbol) return;
        ...
        result.nodes.forEach(n => { n.namespaced = true });
        return result;
    }

    /** Produce a `SpecNode` for given class declaration */
    private _createClassSpecNode(id: string, node: ts.ClassDeclaration): SpecNode | undefined {
        let symbol = node.name && this.checker.getSymbolAtLocation(node.name);
        let typeSymbol = symbol && this.checker.getTypeOfSymbolAtLocation(symbol, symbol.valueDeclaration);
        if (!symbol || !typeSymbol) return;
        ...
        let name = symbol.getName();
        id += (id ? "." : "") + name;
        if (this._nodes[id] && this._nodes[id].type === SpecNodeType.ClassDeclaration) return;
        ...
        result.inherit = node.heritageClauses && node.heritageClauses.map(cl => cl.getText());
    ...
    private _createInterfaceSpecNode(id: string, node: ts.InterfaceDeclaration): SpecNode | undefined {
        let symbol = node.name && this.checker.getSymbolAtLocation(node.name);
        if (!symbol) return;
    ...
    private _createMethodSpecNode(id: string, node: ts.MethodDeclaration) {
        ...
        let name = symbol.getName();
        id += (id ? "." : "") + name;
        if (this._nodes[id] && this._nodes[id].type === SpecNodeType.MethodDeclaration) return;
    ...
    private _recurse(id: string, node: ts.Node): SpecNode[] | SpecNode | undefined {
        ...
        switch (node.kind) {
            ...
            case ts.SyntaxKind.InterfaceDeclaration:
                return this._createInterfaceSpecNode(id, node as ts.InterfaceDeclaration);
            case ts.SyntaxKind.NamespaceExportDeclaration:
            case ts.SyntaxKind.ModuleDeclaration:
                return this._createNamespaceSpecNode(id, node as ts.NamespaceExportDeclaration);
